...
const CreatePoll = () => {
      ...
      questions.map((q, i) => {
        if (i === questionIndex) {
          return {
            ...
            answerOptions:
              answerOptions === null ? q.answerOptions : answerOptions,
          }
        } else {
          return {
            prompt: q.prompt,
  ...
  const updateAnswer = (questionIndex, answerIndex, answerText) => {
    updateQuestion(
      questionIndex,
      null,
      questions[questionIndex].answerOptions.map((a, i) =>
        i === answerIndex ? answerText : a
      )
    )
  }
  ...
  const handleSubmitPoll = async () => {
    ...
    const newPollData = await addPoll({
      title: pollTitle,
      questions: questions,
    })
