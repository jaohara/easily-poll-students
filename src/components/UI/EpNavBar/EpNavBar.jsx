...
import { BiPoll, BiPulse, BiUser } from 'react-icons/bi'
...
const EpNavButton = ({ disabled, route, navHandler }) => {
  ...
  return (
    <EpButton disabled={disabled} onClick={() => navHandler()}>
      {routeIcon && <>{routeIcon} &nbsp;</>}
      {route.name}
    </EpButton>
  )
...
const EpDiagnosticButton = ({ clickHandler }) => {
  return (
    <EpButton className="ep-diagnostic-button" onClick={clickHandler}>
      <BiPulse />
    </EpButton>
  )
}
...
const EpDiagnosticsWindow = ({ diagnosticsData, diagnosticsOpen }) => {
  ...
  const EpDiagnosticsItem = ({ dataKey, dataValue }) => (
    <div className="ep-diagnostics-item" key={`diag-item-${dataKey}`}>
  ...
  return (
    <div className={`ep-diagnostics-window ${diagnosticsOpen ? 'active' : ''}`}>
...
const EpNavBar = () => {
  ...
  const { dumpCurrentAppData } = useContext(AppDataContext)
  ...
  return (
    <div className="ep-nav-bar">
      ...
      <div className="ep-nav-bar-controls">
        ...
        {user && <EpButton onClick={logout}>Logout</EpButton>}
        <EpDiagnosticButton clickHandler={diagnosticClickHandler} />
      </div>
